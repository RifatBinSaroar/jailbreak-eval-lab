...
def validate_dataset(dataset):
    ...
    for case in dataset["cases"]:
        ...
        require(case["split"] in ("development", "pilot", "test"), "case: invalid split")
...
def validate_manifest(manifest, dataset, dataset_hash):
    object_fields(manifest, ("schema_version", "experiment_id", "run_id", "created_at", "split",
                            ...
                            "human_ground_truth", "seed", "ablation"), "manifest", optional=("canonical_experiment_id",))
    if "canonical_experiment_id" in manifest:
        identifier(manifest["canonical_experiment_id"], "canonical experiment ID", r"experiment:[a-z0-9][a-z0-9._-]*")
    ...
    require(manifest["split"] in ("development", "pilot", "test"), "manifest: invalid split")
    require(manifest["purpose"] == {"development": "development", "pilot": "pilot", "test": "final_evaluation"}[manifest["split"]],
            "purpose must match development/pilot/final-test split")
...
def validate_rows(predictions, labels, selected, manifest):
    ...
    for row in predictions:
        ...
        require(row["status"] == "ok" or not row["score"], "indeterminate/error score must remain blank")
