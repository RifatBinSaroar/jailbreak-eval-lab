...
from .canonical import record_canonical, sync_run
...
def main(argv=None):
    ...
    sync = commands.add_parser("sync", help="Recover canonical registration of an immutable bundle")
    sync.add_argument("run_directory")
    sync.add_argument("--directory", default="registries")
    ...
    web.add_argument("--output", default="apps/web/data/research.json")
    ...
    try:
        if args.command == "validate":
            ...
            print(record_canonical(args.manifest, args.dataset, args.predictions, args.labels, args.runs_dir, args.directory, args.project_root))
            ...
            sync_run(args.run_directory, args.directory, args.project_root)
            print("Canonical records match the immutable bundle.")
        else:
            result = rebuild_web(args.runs_dir, args.output, args.include_synthetic)
            print(f"Wrote {args.output}: {result['readiness']['runs']} visible canonical runs")
