...
class FrameworkTests(unittest.TestCase):
    ...
    def record(self):
        return record_run(*self.paths, self.root / "runs")
    ...
    def test_record_bundle_preserves_bytes_and_refuses_overwrite(self):
        run = self.record()
        self.assertEqual({p.name for p in run.iterdir()}, {"manifest.json", "dataset_manifest.json", "predictions.csv", "human_labels.csv", "metrics.json", "errors.json"})
    ...
    def test_observation_tuple_and_quoted_evidence_survive_import(self):
        ...
        run = record_run(*self.paths[:2], predictions_path, self.paths[3], self.root / "runs")
    ...
    def test_web_rebuild_is_deterministic_aggregate_only_and_synthetic_opt_in(self):
        ...
        empty = rebuild_web(self.root / "runs", path)
        self.assertEqual(empty["research_run_count"], 0)
        self.assertEqual(empty["runs"], [])
        result = rebuild_web(self.root / "runs", path, include_synthetic=True)
        self.assertEqual(result["synthetic_run_count"], 1)
        self.assertEqual(result["research_run_count"], 0)
        raw = path.read_bytes()
        ...
        rebuild_web(self.root / "runs", path, include_synthetic=True)
    ...
    def test_ablation_metadata_survives_without_pooled_results(self):
        ...
        record_run(new_manifest, *self.paths[1:], self.root / "runs")
        result = rebuild_web(self.root / "runs", self.root / "web.json", True)
        self.assertEqual(result["runs"][0]["ablation"], manifest["ablation"])
    ...
    def test_failed_export_preserves_previous_file_and_rejects_incomplete_run(self):
        ...
        rebuild_web(self.root / "runs", path)
    ...
    def test_cross_run_dataset_version_conflict(self):
        ...
        record_run(manifest_path, dataset_path, *self.paths[2:], self.root / "runs")
        with self.assertRaisesRegex(ContractError, "conflicting manifests"):
            rebuild_web(self.root / "runs", self.root / "web.json")
    ...
    def test_cross_version_split_reassignment_rejected(self):
        ...
        manifest_path.write_bytes(json_bytes(self.manifest))
        record_run(manifest_path, dataset_path, *self.paths[2:], self.root / "runs")
        with self.assertRaisesRegex(ContractError, "cross-run split leakage"):
            rebuild_web(self.root / "runs", self.root / "web.json")
    ...
    def test_invalid_record_does_not_publish_partial_directory(self):
        ...
        with self.assertRaises(ContractError):
            record_run(manifest_path, *self.paths[1:], self.root / "runs")
        self.assertFalse((self.root / "runs").exists())

    def test_web_output_cannot_overwrite_run_inputs(self):
        run = self.record()
        original = (run / "manifest.json").read_bytes()
        with self.assertRaisesRegex(ContractError, "outside immutable"):
            rebuild_web(self.root / "runs", run / "manifest.json")
    ...
    def test_cli_from_outside_checkout_and_nonzero_error_exit(self):
        ...
        exported = subprocess.run(command + ["rebuild-web", "--runs-dir", str(self.root / "runs"),
