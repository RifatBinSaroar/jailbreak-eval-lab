...
def test_empty_production_is_honest_and_export_is_deterministic(tmp_path):
    empty = {n: {"schema_version":"1.0.0", "registry":n, "records":[]} for n in REGISTRIES}
    directory = save(empty, tmp_path / "registries")
    data = load_registries(directory)
    ...
    assert bundle == web_bundle(directory)
...
@pytest.mark.parametrize("field", ["config", "dataset", "validators", "models", "attack_methods", "code_commit", "timestamp", "split"])
...
def test_computed_fixture_inventory_exports_full_provenance(observed, tmp_path):
    ...
    # Structural validity alone no longer authorizes measured website output.
    validate_registries(observed)
    with pytest.raises(RegistryError, match="immutable recorder bundle"):
        web_bundle(save(observed, tmp_path / "metadata"))
...
def test_website_claims_require_verified_sources_and_keep_buckets(observed, tmp_path):
    ...
    paper['evidence_status'] = 'screened'
    paper['review'] = {'reviewer':'fixture reviewer', 'reviewed_at':paper['timestamp'], 'notes':'Synthetic source review.', 'verified_evidence_ids':[]}
    # Source/evidence display is independently testable with no run findings.
    for name in ('runs','human_labels','results'): observed[name]['records']=[]
    directory = save(observed, tmp_path / "registry")
    bundle = web_bundle(directory)
    assert bundle["verified_paper_evidence"] == []
    assert bundle["paper_evidence"][0]["verification_status"] == 'unverified'
    assert bundle["papers"][0]["our_interpretation"] == paper["our_interpretation"]
    claim["verification_status"] = "verified"
    paper['review']['verified_evidence_ids'] = [claim['id']]
...
def test_malformed_source_reference_rejected(templates):
    ...
    with pytest.raises(RegistryError, match="traceable source|listed intake"):
        validate_registries(templates, allow_templates=True)
...
def test_cli_production_export(tmp_path):
    ...
    assert read_json(output) == web_bundle(ROOT / "registries")
